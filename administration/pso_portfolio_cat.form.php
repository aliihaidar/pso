...
switch ($in2Action) {
    case 'add': 
        ...
        foreach ($_FILES as $uploadedFile) {
            ...
            $uploadedFile['filename'] = 'pocat_img_' . $lastId . '.' . $Extention[1];
            $fileObj = new file2($uploadedFile);
            $fileObj->manipImageUpload('pso_portfolio_cat', $lastId, PROJECTS_CAT_PHOTO_SML, PROJECTS_CAT_PHOTO_MED, PROJECTS_CAT_PHOTO_BIG, true, false, PROJECTS_CAT_PHOTO_MEDBIG, PROJECTS_CAT_PHOTO_LRG, PROJECTS_CAT_PHOTO_BO);
            $fileObj = null;
        }
    ...
    case 'deleteFile':
        $obj->DeleteFile($fileFieldName, array('pocat_id' => $pocat_id), true, true);
        ...
        goToURL($actScript . "?recordId=$pocat_id&in2Action=editRecord&frmAction=edit$error");
        
        break;

    case 'edit': 
			$_REQUEST['pocat_mduser'] = $_SESSION['us_id']; 
			$_REQUEST['pocat_mddate'] = date('Y-m-d H:i:s'); 
        ...
        foreach ($_FILES as $uploadedFile) {
            $Extention = explode('.', $uploadedFile['name']);
            $uploadedFile['filename'] = 'pocat_img_' . $_REQUEST['pocat_id'] . '.' . $Extention[1];
            $fileObj = new file2($uploadedFile);
    ...
    case 'editRecord':
         ...
         */
   if ($pocat_img) {
			        $buttonsImg_pocat_img = '<div class="col-md-3 gallery-pages">
					   						<div class="thumbnail">
					                            <div class="caption">
					                            	<h4>Current Image</h4>
					                                <p>
					                                    <a href="' . PROJECT_UPLOAD_LRG_URL . $pocat_img . '?v=' . rand(100000, 999999) . '"
					                                    ...
					                                    <a href="javascript:deleteFile(\'pocat_img\', \'img\');" rel="tooltip" class="label label-danger" title="Delete">'.$arrLang[$_SESSION['lang']]['remove'].'</a>
					                                </p>
					                            </div>
					                            <img src="' . PROJECT_UPLOAD_BO_URL . $pocat_img . '"/>
					                           </div>
					                          </div>';
		...
		$recordDetails = '<div class="panel">
                                ...
                                <div class="panel-body">
                                	<h4>Create</h4>
                                	<p>
                                		<i aria-hidden="true" class="icon-user"> '.$pocat_cruser.'</i><br/><i aria-hidden="true" class="icon-calendar"> '.$pocat_crdate.'</i>
                                	</p>
                                	<hr/>
                                	<h4>Update</h4>
                                	<p>
                                		<i aria-hidden="true" class="icon-user"> '.$pocat_mduser.'</i><br/><i aria-hidden="true" class="icon-calendar"> '.$pocat_mddate.'</i>
                                	</p>
                                </div>
                            </div>';
...
<head><title>Dashboard | Dashboard</title>
						                        	...
						                        	<div class="col-md-9">
						                                <div class="panel">
						                                    ...
						                                    <div class="panel-body">
											                        ...
											                        <input type="hidden" name="pocat_id" id="pocat_id" value="<?php print $pocat_id ?>" />
	...
	<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['pocat_title']; ?></label>
										            <div class="col-md-10">
										                <input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['pocat_title']; ?>"  class="form-control input-large" name="pocat_title" id="pocat_title" value="<?php echo htmlentities($pocat_title, ENT_NOQUOTES) ?>" />
										            </div> </div>
  
  
<div class="form-group">
	<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['pocat_order']; ?></label>
										            <div class="col-md-10">
										                <input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['pocat_order']; ?>"  class="form-control input-large" name="pocat_order" id="pocat_order" value="<?php echo number_format(htmlentities($pocat_order, ENT_NOQUOTES), 0, DECIMAL_SEPERATOR, THOUSAND_SEPERATOR) ?>" />
										            </div> </div>
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                        	<div class="form-group">
                        		<div class="col-md-6">
                        			<label class="control-label"><?php echo $arrLang[$_SESSION['lang']]['pocat_published']; ?></label>
                        		</div>
                        		<div class="col-md-6">
                        			<div class="form-group">
                        				<div data-on="success" data-off="danger" data-on-label="YES" data-off-label="NO" class="make-switch">
                                 							<input name="pocat_published" id="pocat_published" type="checkbox" class="switch"/>
                                 						</div>
                        			</div>
                        		</div>
                        		<div class="col-md-12">
	...
	if(<?php echo ($pocat_published ? $pocat_published : 0) ?> == 1){
						$('#pocat_published').parent().bootstrapSwitch('setState' , true);
					}
					
					$('#pocat_published').parent().on('change', function(event,  state) {
				        if($('#pocat_published').parent().bootstrapSwitch('status')) $('#pocat_published').attr('checked', 'checked')
				        else $('#pocat_published').removeAttr('checked')
				    });
