<?php
	...
	****************************************/
	 ...
	 class MrThumb {
		...
		public function contrain ( $width, $height ) {
			if ( ! ( $this->halt ) ) {
				if ( $this->image['extension'] == 'gif' ) {
					...
					imagecopyresampled( $this->image['composite'], $this->image['render'], 0, 0, 0, 0, $width, $height, $this->image['width'], $this->image['height'] );
				    ...
				    //  Below condition block added by JBC on 2010-02-01 @ 11:00 AM
				    if ($this->image['extension'] == 'png') {
                        imagealphablending($this->image['composite'], false);
                        imagesavealpha($this->image['composite'], true);
				    }
				...
				/**
				 * Added by JBC on 2009-12-27
				 */
				$this->image['widthNew']   = $width;
				$this->image['heightNew']  = $height;
			} else {
		...
		public function proportion ($max_width = null, $max_height = null) {

			if ( ! ( $this->halt ) && ($max_width || $max_height)) {
			    if ($max_width && $max_height) {
                    $this->image['ratio']   = ($this->image['width'] > $this->image['height']) ? $max_width / $this->image['width'] : $max_height / $this->image['height'];
			    } elseif ($max_width) {
			        $this->image['ratio']   = $max_width / $this->image['width'];
			    } else {
			        $this->image['ratio']   = $max_height / $this->image['height'];
			    }
				 ...
				 * Added by JBC on 2009-12-27
				 */
				$this->image['widthNew']   = $new_width;
				$this->image['heightNew']  = $new_height;
			} else {
				echo 'Execution halted!';
			}
        ...
        public function saveto ($destination, $filename, $quality = 100, $withExtension = true) {
			...
			if ( ! ( $this->halt ) ) {
				...
				switch ( $this->image['extension'] ) {
					case 'png';
						imagepng( $this->image['composite'], $destination . $filename . $extension, null );
					break;
					case 'jpg';
						imagejpeg( $this->image['composite'], $destination . $filename . $extension, $quality );
					break;
					case 'jpeg';
						imagejpeg( $this->image['composite'], $destination . $filename . $extension, $quality );
					break;
					case 'gif';
						imagegif( $this->image['composite'], $destination . $filename . $extension, $quality );
					break;
					case 'bmp';
						imagewbmp( $this->image['composite'], $destination . $filename . $extension, null );
					break;
					case 'wbmp';
						imagewbmp( $this->image['composite'], $destination . $filename . $extension, null );
					break;
