...
class MySQL
{
	...
	private $db_dbname  = DBNAME;           // database name
	...
	const SQLVALUE_DOC      = "doc";
	const SQLVALUE_IMG      = "img";
	const SQLVALUE_VID      = "vid";
	 ...
	 */
	public function __construct($connect=true, $database="", $server="", $username="", $password="", $charset="", $pconnect="false") {
		...
		$pconnect ? $this->db_pcon    = true : $this->db_pcon = false;
	...
	public function importConnection($obj){
		$obj->cloneObject($this);
	}
	Public function cloneObject($obj){
		$obj->cloneObjectProperties($this->db_host, $this->db_user, $this->db_pass, $this->db_dbname, $this->db_charset, $this->db_pcon, $this->mysql_link);
	}
	Public function cloneObjectProperties($db_host, $db_user, $db_pass, $db_dbname, $db_charset, $db_pcon, $mysql_link){
			$this->db_host = $db_host;
			$this->db_user = $db_user;
			$this->db_pass = $db_pass;
			$this->db_dbname = $db_dbname;
			$this->db_charset = $db_charset;
			$this->db_pcon = $db_pcon;
    ...
    static public function BuildSQLDeleteLogical($tableName, $tableId, $whereArray = null) {
        $tableIdKeys    = array_keys($tableId);
        $tableIdTmp     = $tableIdKeys[0];
        $tableId        = split('_', $tableIdTmp);

        $tablePrefix    = $tableId[0];

		$sql = "UPDATE $tableName
                SET {$tablePrefix}". DELETE_FIELD_NAME ." = 1";
   ...
   /**
	...
	public function SelectDatabase($database, $charset = "") {
		...
		if (! (mysql_select_db($database))) {
			...
			if ((strlen($charset) > 0)) {
                /**
                 * Modified by JBC on 2010-12-22 @ 01:45 PM
                 * To fix charset bug.
                 */
