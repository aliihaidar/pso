...
switch ($in2Action) {
	...
	foreach ($_FILES as $uploadedFile) {
		...
		$uploadedFile['filename'] = 'us_img_' . $lastId . '.' . $Extention[1];
		$fileObj = new file2($uploadedFile);
		$fileObj->manipImageUpload('usr', $lastId, FEATURED_PHOTO_SML, FEATURED_PHOTO_MED, FEATURED_PHOTO_BIG, true, false, FEATURED_PHOTO_MEDBIG, FEATURED_PHOTO_LRG, FEATURED_PHOTO_BO);
		$fileObj = null;
	}
	...
	case 'deleteFile':
	$obj->DeleteFile($fileFieldName, array('us_id' => $us_id), true, true);
	...
	goToURL($actScript . "?recordId=$us_id&in2Action=editRecord&frmAction=edit$error");
	
	break;

	case 'edit': 
	$_REQUEST['us_mduser'] = $_SESSION['us_id']; 
	$_REQUEST['us_mddate'] = date('Y-m-d H:i:s');
	$_REQUEST['us_password'] = md5($_REQUEST['us_password']); 
	...
	foreach ($_FILES as $uploadedFile) {
		$Extention = explode('.', $uploadedFile['name']);
		$uploadedFile['filename'] = 'us_img_' . $_REQUEST['us_id'] . '.' . $Extention[1];
		$fileObj = new file2($uploadedFile);
		$fileObj->manipImageUpload('usr', $_REQUEST['us_id'], FEATURED_PHOTO_SML, FEATURED_PHOTO_MED, FEATURED_PHOTO_BIG, true, false, FEATURED_PHOTO_MEDBIG, FEATURED_PHOTO_LRG, FEATURED_PHOTO_BO);
		$fileObj = null;
	}
	...
	goToURL($actScript . "?recordId=".$_REQUEST['us_id']."&in2Action=editRecord&frmAction=edit$error");
	...
	$buttons .= '<button type="button" class="btn btn-block btn-default btn-circle" onclick="window.location=\''.$actScript.'\'"><i class="fa fa-plus"></i>&nbsp;'.$arrLang[$_SESSION['lang']]['addnew'].'</button>';
         ...
         */
        if ($us_img) {
        	$buttonsImg_us_img = '<div class="col-md-3 gallery-pages">
        	<div class="thumbnail">
        	<div class="caption">
        	<h4>Current Image</h4>
        	<p>
        	<a href="' . PROJECT_UPLOAD_LRG_URL . $us_img . '?v=' . rand(100000, 999999) . '"
        	...
        	<a href="javascript:deleteFile(\'us_img\', \'img\');" rel="tooltip" class="label label-danger" title="Delete">'.$arrLang[$_SESSION['lang']]['remove'].'</a>
        	</p>
        	</div>
        	<img src="' . PROJECT_UPLOAD_BO_URL . $us_img . '"/>
        	</div>
        	</div>';
        ...
        $sql = "SELECT CONCAT(us_fname,' ',us_lname) as createdBy FROM usr WHERE us_id = $us_cruser";
        ...
        $sql = "SELECT CONCAT(us_fname,' ',us_lname) as updatedBy FROM usr WHERE us_id = $us_mduser";
        $result = $conn->query($sql);
        $updatedBy = '';
        ...
        <h4>Created By:</h4>
        <p>
        <i aria-hidden="true" class="icon-user"> '.$createdBy.'</i><br/><i aria-hidden="true" class="icon-calendar"> '.$us_crdate.'</i>
        </p>
        <hr/>
        <h4>Updated By:</h4>
        <p>
        <i aria-hidden="true" class="icon-user"> '.$updatedBy.'</i><br/><i aria-hidden="true" class="icon-calendar"> '.$us_mddate.'</i>
        </p>
        </div>
        </div>';
									...
									<div class="col-md-9">
										<div class="panel">
											...
											<div class="panel-body">
												...
												<input type="hidden" name="us_id" id="us_id" value="<?php print $us_id ?>" />
												...
												<div class="form-body">
													...
													<div class="form-group">
														<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_lname']; ?></label>
														<div class="col-md-10">
															<input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['us_lname']; ?>"  class="form-control input-large" name="us_lname" id="us_lname" value="<?php echo htmlentities($us_lname, ENT_NOQUOTES) ?>" />
														</div> </div>
														
														<div class="form-group">
															<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_fname']; ?></label>
															<div class="col-md-10">
																<input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['us_fname']; ?>"  class="form-control input-large" name="us_fname" id="us_fname" value="<?php echo htmlentities($us_fname, ENT_NOQUOTES) ?>" />
															</div> </div>
															
															<div class="form-group">
																<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_img']; ?></label>
																<div class="col-md-10">
																	<?php echo $buttonsImg_us_img ?>
																	<input type="file" name="us_img" id="us_img" class="form-control input-medium" value="" />
																</div> </div>
																
																<div class="form-group">
																	<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_dob']; ?></label>
																	<div class="col-md-6">
																		<div class="input-group datetimepicker-disable-time date">
																			<input type="text" name="us_dob" id="us_dob" class="form-control" data-date-format="mm/dd/yyyy" value="<?php date_format(new DateTime(htmlentities($us_dob, ENT_NOQUOTES)),"m/d/Y"); ?>"/>
																...
																<div class="form-group">
																	<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_address']; ?></label>
																	<div class="col-md-10">
																		<input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['us_address']; ?>"  class="form-control input-large" name="us_address" id="us_address" value="<?php echo htmlentities($us_address, ENT_NOQUOTES) ?>" />
																	</div> </div>
																	
																	<div class="form-group">
																		<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_phone']; ?></label>
																		<div class="col-md-10">
																			<input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['us_phone']; ?>"  class="form-control input-large" name="us_phone" id="us_phone" value="<?php echo htmlentities($us_phone, ENT_NOQUOTES) ?>" />
																		</div> </div>
																		
																		<div class="form-group">
																			<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_mobile']; ?></label>
																			<div class="col-md-10">
																				<input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['us_mobile']; ?>"  class="form-control input-large" name="us_mobile" id="us_mobile" value="<?php echo htmlentities($us_mobile, ENT_NOQUOTES) ?>" />
																			</div> </div>
																			
																			<div class="form-group">
																				<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_email']; ?></label>
																				<div class="col-md-10">
																					<input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['us_email']; ?>"  class="form-control input-large" name="us_email" id="us_email" value="<?php echo htmlentities($us_email, ENT_NOQUOTES) ?>" />
																				</div> </div>
																				
																				<div class="form-group">
																					<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_username']; ?></label>
																					<div class="col-md-10">
																						<input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['us_username']; ?>"  class="form-control input-large" name="us_username" id="us_username" value="<?php echo htmlentities($us_username, ENT_NOQUOTES) ?>" />
																					</div> </div>
																					
																					<div class="form-group">
																						<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_password']; ?></label>
																						<div class="col-md-10">
																							<input type="password" placeholder="<?php echo $arrLang[$_SESSION['lang']]['us_password']; ?>"  class="form-control input-large" name="us_password" id="us_password" />
																						</div> </div>
																						
																						<div class="form-group">
																							<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_isactive']; ?></label>
																							<div class="col-md-10">
																								<div data-on="success" data-off="danger" data-on-label="YES" data-off-label="NO" class="make-switch">
																									<input name="us_isactive" id="us_isactive" type="checkbox" class="switch"/>
																								</div>
																							</div> </div>
																							
																							<div class="form-group">
																								<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['us_gr_id']; ?></label>
																								<div class="col-md-10">
																									<?php echo genTableList('us_gr_id', 'grp', null, 'gr_id', 'gr_title', false, true, $arrLang[$_SESSION['lang']]['genTableSelect'], htmlentities($us_gr_id, ENT_NOQUOTES), 'form-control input-large'); ?>
																								</div> </div>
...
if(<?php echo ($us_isactive ? $us_isactive : 0) ?> == 1){
	$('#us_isactive').parent().bootstrapSwitch('setState' , true);
}

$('#us_isactive').parent().on('change', function(event,  state) {
	if($('#us_isactive').parent().bootstrapSwitch('status')) $('#us_isactive').attr('checked', 'checked')
		else $('#us_isactive').removeAttr('checked')
	});
