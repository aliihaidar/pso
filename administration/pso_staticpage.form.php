...
switch ($in2Action) {
    case 'add': 
        ...
        foreach ($_FILES as $uploadedFile) {
            ...
            $uploadedFile['filename'] = 'sp_img_' . $lastId . '.' . $Extention[1];
            $fileObj = new file2($uploadedFile);
            $fileObj->manipImageUpload('pso_staticpage', $lastId, STATICPAGE_PHOTO_SML, STATICPAGE_PHOTO_MED, STATICPAGE_PHOTO_BIG, true, false, STATICPAGE_PHOTO_MEDBIG, STATICPAGE_PHOTO_LRG, STATICPAGE_PHOTO_BO);
            $fileObj = null;
        }
    ...
    case 'deleteFile':
        $obj->DeleteFile($fileFieldName, array('sp_id' => $sp_id), true, true);
        ...
        goToURL($actScript . "?recordId=$sp_id&in2Action=editRecord&frmAction=edit$error");
        
        break;

    case 'edit': 
			$_REQUEST['sp_mduser'] = $_SESSION['us_id']; 
			$_REQUEST['sp_mddate'] = $_SESSION['us_id']; 
        ...
        foreach ($_FILES as $uploadedFile) {
            $Extention = explode('.', $uploadedFile['name']);
            $uploadedFile['filename'] = 'sp_img_' . $_REQUEST['sp_id'] . '.' . $Extention[1];
            $fileObj = new file2($uploadedFile);
            $fileObj->manipImageUpload('pso_staticpage', $_REQUEST['sp_id'],STATICPAGE_PHOTO_SML, STATICPAGE_PHOTO_MED, STATICPAGE_PHOTO_BIG, true, false, STATICPAGE_PHOTO_MEDBIG, STATICPAGE_PHOTO_LRG, STATICPAGE_PHOTO_BO);
            $fileObj = null;
        }
        ...
        goToURL($actScript . "?recordId=".$_REQUEST['sp_id']."&in2Action=editRecord&frmAction=edit$error");
    ...
    case 'editRecord':
        ...
        if ($id) {
            $where     = "WHERE sp_id = " . $obj->getSqlColValue('sp_id', $id);
         ...
         */
   if ($sp_img) {
			        $buttonsImg_sp_img = '<div class="col-md-3 gallery-pages">
					   						<div class="thumbnail">
					                            <div class="caption">
					                            	<h4>Current Image</h4>
					                                <p>
					                                    <a href="' . PROJECT_UPLOAD_LRG_URL . $sp_img . '?v=' . rand(100000, 999999) . '"
					                                    ...
					                                    <a href="javascript:deleteFile(\'sp_img\', \'img\');" rel="tooltip" class="label label-danger" title="Delete">'.$arrLang[$_SESSION['lang']]['remove'].'</a>
					                                </p>
					                            </div>
					                            <img src="' . PROJECT_UPLOAD_BO_URL . $sp_img . '"/>
					                           </div>
					                          </div>';
		...
		$recordDetails = '<div class="panel">
                                ...
                                <div class="panel-body">
                                	<h4>Create</h4>
                                	<p>
                                		<i aria-hidden="true" class="icon-user"> '.$sp_cruser.'</i><br/><i aria-hidden="true" class="icon-calendar"> '.$sp_crdate.'</i>
                                	</p>
                                	<hr/>
                                	<h4>Update</h4>
                                	<p>
                                		<i aria-hidden="true" class="icon-user"> '.$sp_mduser.'</i><br/><i aria-hidden="true" class="icon-calendar"> '.$sp_mddate.'</i>
                                	</p>
                                </div>
                            </div>';
...
<head><title>PSO | Dashboard</title>
						                        	...
						                        	<div class="col-md-9">
						                                <div class="panel">
						                                    ...
						                                    <div class="panel-body">
											                        ...
											                        <input type="hidden" name="sp_id" id="sp_id" value="<?php print $sp_id ?>" />
	...
	<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['sp_type']; ?></label>
										            <div class="col-md-10">
										            	<?php echo genArrayList('sp_type', array('about'=>$arrLang[$_SESSION['lang']]['sp_type_about'],'privacy'=>$arrLang[$_SESSION['lang']]['sp_type_privacy'],'terms'=>$arrLang[$_SESSION['lang']]['sp_type_terms'],'other'=>$arrLang[$_SESSION['lang']]['sp_type_other']), false, $arrLang[$_SESSION['lang']]['all'], htmlentities($sp_type, ENT_NOQUOTES), 'form-control input-large')?>
										            </div> </div>
  
<div class="form-group">
	<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['sp_title']; ?></label>
										            <div class="col-md-10">
										                <input type="text" placeholder="<?php echo $arrLang[$_SESSION['lang']]['sp_title']; ?>"  class="form-control input-large" name="sp_title" id="sp_title" value="<?php echo htmlentities($sp_title, ENT_NOQUOTES) ?>" />
										            </div> </div>
  
<div class="form-group">
<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['sp_img']; ?></label>
											            <div class="col-md-10">
											            	<?php echo $buttonsImg_sp_img ?>
											            	<input type="file" name="sp_img" id="sp_img" class="form-control input-medium" value="" />
											            </div> </div>
  
<div class="form-group">
	<label class="col-md-2 control-label"><?php echo $arrLang[$_SESSION['lang']]['sp_desc']; ?></label>
										            <div class="col-md-10">
										            	<textarea id="content" rows="10" class="form-control" name="sp_desc" id="sp_desc"><?php echo htmlentities($sp_desc, ENT_NOQUOTES) ?></textarea>
										            </div> </div>
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                        	<div class="form-group">
                        		<div class="col-md-6">
                        			<label class="control-label"><?php echo $arrLang[$_SESSION['lang']]['sp_published']; ?></label>
                        		</div>
                        		<div class="col-md-6">
                        			<div class="form-group">
                        				<div data-on="success" data-off="danger" data-on-label="YES" data-off-label="NO" class="make-switch">
                                 							<input name="sp_published" id="sp_published" type="checkbox" class="switch"/>
                                 						</div>
                        			</div>
                        		</div>
                        		<div class="col-md-12">
	...
	if(<?php echo ($sp_published ? $sp_published : 0) ?> == 1){
						$('#sp_published').parent().bootstrapSwitch('setState' , true);
					}
					
					$('#sp_published').parent().on('change', function(event,  state) {
				        if($('#sp_published').parent().bootstrapSwitch('status')) $('#sp_published').attr('checked', 'checked')
				        else $('#sp_published').removeAttr('checked')
				    });
